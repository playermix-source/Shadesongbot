...
# LastFM key - set via env var or fallback to hardcoded
LASTFM_KEY = os.environ.get("LASTFM_KEY", "c9b16bfc1f90c14d1e3b20a5d7c2fead")

# Penalty words for scoring
PENALTY_WORDS = [
...
def _find_best_match(results, query):
    ...
    for song in results:
        ...
        # ==== KEY FIX: heavily penalize short clips ====
        duration = int(song.get("duration", 0))
        if 0 < duration < 90:
            score -= 200  # basically never pick this
...
def _saavn_dev(query, limit=10):
    try:
        ...
        for s in results:
            ...
            # Skip short clips from search results too
            if 0 < duration < 90:
                continue
...
def _saavn_quality(query, quality="320", limit=10):
    """
    Get best quality full song (>=90s) from JioSaavn.
    Returns None if not found or only clips available.
    """
    # Try saavn.dev first
    try:
        r = requests.get(
        ...
        if r.status_code == 200:
            results = r.json().get("data", {}).get("results", [])
            # Only keep full songs (>=90s)
            full_results = [x for x in results if int(x.get("duration", 0)) >= 90]
            if full_results:
                mapped = [{
                    "name": x.get("name", ""),
                ...
                if s:
                    raw = s.get("_raw") or full_results[0]
                    ...
                    if dl_url:
                        ...
                        print(f"[saavn.dev] ✅ {raw.get('name')} ({duration}s) | {dl_url[:50]}")
                        return {
                            "source": "jiosaavn",
    ...
    try:
        ...
        if r2.status_code == 200:
            ...
            full_results_old = [x for x in results_old if int(x.get("duration", 0)) >= 90]
            if full_results_old:
                mapped = [{
                    "name": x.get("name", ""),
                    ...
                    "duration": int(x.get("duration", 0)),
                    "_raw": x
                } for x in full_results_old]
                s = _find_best_match(mapped, query)
                if s:
                    raw = s.get("_raw") or full_results_old[0]
                    dl_urls = raw.get("downloadUrl", [])
                    ...
                    if dl_url:
                        ...
                        album_str = album_raw.get("name", "Unknown") if isinstance(album_raw, dict) else str(album_raw or "Unknown")
                        duration = int(raw.get("duration", 0))
                        print(f"[saavn_old] ✅ {raw.get('name')} ({duration}s) | {dl_url[:50]}")
                        return {
                            "source": "jiosaavn",
                            "name": raw.get("name", "Unknown"),
    ...
    print(f"[saavn_quality] Both APIs returned only clips or nothing for: {query}")
    return None
...
def detect_language(query):
    ...
    hindi_words = {
        ...
        "dheere", "nazar", "aankhein", "aankhon", "baahon",
        "o", "oh", "oo", "re", "ri",
        ...
        "talwiinder", "talwinder", "praak", "jordan",
        "imran", "rahat", "sonu", "kumar", "udit", "lata", "asha",
        "kishore", "rafi", "mukesh", "hemant", "manna",
        "badshah", "honey", "diljit", "neha", "guru", "sunidhi",
    }
...
def _score_all(results, query):
    ...
    for song in results:
        ...
        # ==== KEY FIX: heavily penalize short clips in display results too ====
        duration = int(song.get("duration", 0))
        if 0 < duration < 90:
            score -= 200
        elif duration > 150:
...
def search_songs(query, limit=10):
    """Smart multi-API search — always returns full songs only"""
...
def search_song_download(query, quality="320"):
    """
    Get best downloadable FULL song (>=90s).
    Flow:
      1. JioSaavn full song (>=90s, 320kbps)
      2. yt-dlp YouTube Music (if JioSaavn short/missing)
      3. Last resort: whatever is available
    """
    # 1. Try JioSaavn — only returns >=90s songs now
    song = _saavn_quality(query, quality)
    saavn_duration = int(song.get("duration", 0)) if song else 0

    if song and saavn_duration >= 90:
        print(f"[saavn] ✅ Full song: {song.get('name')} ({saavn_duration}s)")
        return song

    # JioSaavn returned nothing or a clip — try yt-dlp
    if song and saavn_duration > 0:
        print(f"[saavn] ⚠️ Only clip ({saavn_duration}s) for '{query}' — trying yt-dlp first")
    else:
        print(f"[saavn] ❌ Not found '{query}' — trying yt-dlp")

    # 2. yt-dlp YouTube Music
    yt_song = _ytdlp_download(query)
    if yt_song:
        print(f"[yt-dlp] ✅ {yt_song.get('name')} ({yt_song.get('duration')}s)")
        return yt_song

    # 3. JioSaavn clip better than nothing
    if song:
        print(f"[saavn] ⚠️ Returning short clip as absolute last resort")
        return song

    # 4. Deezer/iTunes 30sec preview (absolute last resort)
...
def _ytdlp_download(query):
    ...
    Returns song dict with '_local_path' pointing to downloaded file.
    """
    try:
        import yt_dlp
    except ImportError:
    ...
    try:
        ...
        ydl_opts = {
            ...
            # Only download full songs — reject shorts/clips
            "match_filter": lambda info, *, incomplete=False: (
                "Too short" if (info.get("duration") or 0) < 90 else None
            ),
        }

        search_query = f"ytmsearch3:{query}"  # get 3 candidates, pick best
        ...
        # Pick the entry with longest duration (most likely full song)
        entry = None
        for e in entries:
            if e and int(e.get("duration") or 0) >= 90:
                if entry is None or int(e.get("duration", 0)) > int(entry.get("duration", 0)):
                    entry = e
        if not entry:
            print(f"[yt-dlp] No full song found for: {query}")
            return None
...
def _ytdlp_search_multiple(query, limit=6):
    ...
    Metadata only — no download. Only returns full songs (>=90s).
    """
    try:
        import yt_dlp
    except ImportError:
    ...
    try:
        ...
        search_query = f"ytmsearch{limit + 3}:{query}"  # get a few extra to filter
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
