...
def _saavn_quality(query, quality="320", limit=10):
    ...
    try:
        ...
        if r.status_code == 200:
            results = r.json().get("data", {}).get("results", [])
            if results:
                # Build mapped list WITH duration so _find_best_match can score/penalize
                mapped = [{
                    "name": x.get("name", ""),
                ...
                # Filter out short clips BEFORE scoring — don't even consider them
                full_songs = [m for m in mapped if int(m.get("duration", 0)) >= 90]
                pool = full_songs if full_songs else mapped  # fallback to all if nothing ≥90s
                ...
                if s:
                    ...
                    # Hard reject short clips — let yt-dlp handle it
                    if dur > 0 and dur < 90:
                        print(f"[saavn.dev] ⚠️ Best match is short clip ({dur}s): {raw.get('name')} — skipping")
                        return None
                    ...
                    if dl_url:
                        ...
                        album_str = album_raw.get("name", "Unknown") if isinstance(album_raw, dict) else str(album_raw or "Unknown")
                        print(f"[saavn.dev] ✅ {raw.get('name')} ({dur}s) | {dl_url[:50]}")
                        return {
                            "source": "jiosaavn",
        ...
        if r2.status_code == 200:
            ...
            if results_old:
                ...
                if s:
                    ...
                    if dur > 0 and dur < 90:
                        print(f"[saavn_old] ⚠️ Best match is short clip ({dur}s): {raw.get('name')} — skipping")
                        return None
                    ...
                    if dl_url:
                        album_raw = raw.get("album", {})
                        album_str = album_raw.get("name", "Unknown") if isinstance(album_raw, dict) else str(album_raw or "Unknown")
                        print(f"[saavn_old] ✅ {raw.get('name')} ({dur}s) | {dl_url[:50]}")
                        return {
                            "source": "jiosaavn",
                            "name": raw.get("name", "Unknown"),
    ...
    print(f"[saavn_quality] Both APIs failed or only short clips for: {query}")
    return None
